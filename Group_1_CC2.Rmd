...
positpal::start("Group_1_CC2.Rmd")
```
...
1. **Li Hung** — 20494782
    + [BIOL343_CC](https://github.com/ApplesofEarth/BIOL343_CC)
1. **Grant Bishop** — 20485713
    + [BIOL343_CC](github.com/bishg1/BIOL_343_CC)
1. **Oona Esme** - 20307366
    + [BIOL343_CC](github.com/esme-o/Biol343_CC2)
1. **Archi** - 20475426
    + [BIOL343_CC](https://github.com/arch1420/BIOL343)
1. **Ella** — 20457793
...
---------|---------------|--------
population_type | Indiciates the asexual or sexual type  | no units
...
cc1_data.uncleaned <- read.csv("./A1_Data_Excel.csv", header = T)
cc1_data = cc1_data.uncleaned[1:46, 1:8]
...
$$
\text{Ovary Height} \times \text{Ovary Width} = \text{Ovary Area}
$$
The units for height and width are both in $mm$ while area is in $mm^2$
...
kable(AreaTable, caption = "The means of ovary area grouped by population types: Asexual and Sexual")
...
```{r area-histogram, echo = FALSE}

library(ggplot2)
ggplot(aes(x=ovary_area), data=cc1_data) + geom_histogram(binwidth = 1) + facet_wrap(.~population_type) + xlab(expression("Ovary Area (mm"^2*")")) + labs(title="Distribution of Ovary Areas") + theme_classic()
...
![Solanum pimpinellifolium](./images/Hydroponic_g13.jpg)
